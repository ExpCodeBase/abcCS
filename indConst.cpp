...
void skyIndexBasic(BiGraph& g, unordered_map<string, vector<vector<int>>>& skyLable) {
...
void skyIndexDelta(BiGraph& g, unordered_map<string, vector<vector<int>>>& skyLable, int &delta) {
	int left_degree_max = 0;
	for (int i = 0; i < g.getV1Num(); i++) {
		if (left_degree_max < g.getV1Degree(i)) left_degree_max = g.getV1Degree(i);
	}
					...
					for(auto e: g.getV1Neighbors(vtid)){
						if(right_pushed[e]){
							...
							if(!tightFlag && !skyline_index_v2par[nowNei]){
								for(auto e: skyline_index_v2h[nowNei]){
									if(e->hubid[1] == cur_bd->right_c->hubid[1]){
										if(e->hubid[0] < minUntight){
											minUntight = e->hubid[0];
											untight = nowNei;		
										}
										closeFlag = false;
										break;
...
void retrieve_skyline_index_lv0(BiGraph& g, 
	...
	cout << "Query done for " << "(" << a_q << ", " << b_q <<")-core." <<endl;
...
void retrieve_skyline_index_lv1(BiGraph& g, 
	...
	cout << "Query done for " << "(" << a_q << ", " << b_q <<")-core of " << v_q <<endl;
